...
## (현재 운영 중) 1. RoastLink — 커피 로스터기 ↔ Artisan 브리지 프로그램
...
현재 웹사이트 운영, 주변의 로스터들에게 직접 프로그램을 전달하여 피드백 접수, 제가 구상한 유료 기능(아직 비공개)과 피드백을 반영한 v2 개발을 위한 설계 명세 중입니다. (사이트 정적 페이지를 생성하는 빌드 도구는 Python에서 Java로 교체했습니다.)
